<template>
  <div
    ...
    <div v-if="!isProjectOpen"
      ...
      <div
        class="absolute -right-10 -top-10 w-64 h-64 bg-[#f472b6] dark:bg-[#111] rounded-full border-4 border-black z-0 opacity-50 dark:border-[#222]">
      </div>

      <h3
        class="text-4xl font-black uppercase mb-4 relative z-10 bg-white px-4 py-2 border-4 border-black dark:bg-[#0a0a0a] dark:border-[#222]">
        Open Your Project
      </h3>
    ...
    <div v-else class="flex-1 flex flex-col min-h-0 bg-[#f4f4f0] dark:bg-[#050505]">
      ...
      <div v-show="activeMainTab === 'editor'" class="flex-1 flex flex-row min-h-0 p-3 gap-3">
        ...
        <!-- Activity Bar -->
        <div
          class="w-16 bg-white border-4 border-black flex flex-col items-center py-4 shrink-0 justify-between z-10 dark:bg-[#0a0a0a] dark:border-[#222]">
          ...
          <div v-show="activeSidebar === 'explorer'" class="flex-1 flex flex-col min-h-0">
            <div
              ...
              <span class="flex items-center gap-2">
                Explorer
                <i @click="refreshExplorer" class="pi pi-refresh cursor-pointer hover:scale-125 transition-transform"
                  title="Refresh Explorer / Scan Project"></i>
            ...
            <div id="explorer-container"
              class="flex-1 overflow-y-auto custom-scrollbar bg-[#f4f4f0] dark:bg-[#050505] flex flex-col py-2"
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-2 bg-[#f4f4f0] dark:bg-[#050505]">
              ...
              <template v-else>
                <div v-for="result in searchResults" :key="result.path"
                  class="mb-4 border-4 border-black bg-white dark:border-[#222] dark:bg-[#0a0a0a]">
                  <div
                    ...
                    <span class="ml-auto bg-neo-pink text-black dark:bg-white dark:text-black px-2 py-1 font-bold">{{
                  ...
                  <div class="flex flex-col bg-white dark:bg-[#0a0a0a]">
                    <div v-for="match in result.matches" :key="match.line"
                      ...
                      <span class="font-bold shrink-0 w-6 text-right text-gray-500 dark:text-gray-400">{{ match.line
                        }}</span>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
              ...
              <div class="border-b-4 border-black bg-white dark:border-[#222] dark:bg-[#0a0a0a]">
                ...
                <div v-show="isInstalledPluginOpen" class="flex flex-col bg-white dark:bg-[#0a0a0a]">
                  <div v-if="installedList.length === 0"
                    class="p-4 text-center text-xs font-bold text-gray-500 dark:text-gray-400">
                  ...
                  <div v-for="plugin in installedList" :key="plugin.id"
                    ...
                    <div class="flex flex-col flex-1 min-w-0">
                      ...
                      <div class="text-xs text-gray-600 truncate mt-0.5 dark:text-gray-400">{{ plugin.description }}
                      </div>
                      <div
                        class="text-[10px] text-gray-500 font-bold mt-1.5 flex justify-between items-center dark:text-gray-400">
                        {{ plugin.author }}</div>
                    </div>
                  </div>
                </div>
        ...
        <div
          class="flex-1 flex flex-col min-w-0 z-10 h-full border-4 border-black bg-white dark:border-[#222] dark:bg-[#0a0a0a]">
          <!-- Toolbar -->
          <div
            ...
            <div
              ...
              <span v-if="unsavedChanges[selectedFile]"
                class="bg-red-500 w-3 h-3 rounded-full border-2 border-black shrink-0 dark:border-[#222]"
          ...
          <div class="flex-1 flex flex-row min-h-0 bg-white relative dark:bg-[#0a0a0a]" ref="editorSplitRef">
            <div v-if="renderError"
              ...
              <h2
                ...
                Rendering Error</h2>
              <div
                class="bg-white text-black p-4 border-4 border-black font-bold dark:bg-[#0a0a0a] dark:text-gray-100 dark:border-[#222]">
                {{ renderError }}</div>
            ...
            <div class="h-full min-w-0 flex flex-col relative bg-[#f4f4f0] dark:bg-[#050505]"
              ...
              <div v-if="openTabs.length > 0"
                ...
                <div v-for="tab in openTabs" :key="tab" @click="openFile(tab)"
                  ...
                  <i class="pi text-sm shrink-0"
                    :class="[selectedFile === tab ? 'text-white' : 'text-gray-500 group-hover:text-black', tab.startsWith('plugin:') ? getPluginIcon(tab.replace('plugin:', '')) : getFileIcon(tab.split('/').pop() || '').icon]"></i>
              ...
              <div v-show="isDiffMode && currentDiffFile" class="flex-1 min-h-0 bg-[#1e1e1e] relative flex flex-col">
                <div
                  class="py-3 bg-white border-b-4 border-black shrink-0 flex items-center justify-between px-4 dark:bg-[#0a0a0a] dark:border-[#222]">
                  <div class="font-mono text-sm font-black flex flex-col gap-3 text-black dark:text-gray-100">
                    <div class="flex items-center gap-2 mt-1">
                      <span
                        class="bg-[#ef4444] text-white px-2 py-1 border-2 border-black dark:border-[#222]">REVIEWING</span>
              ...
              <div v-show="!selectedFile"
                ...
                <div
                  class="bg-white border-4 border-black p-8 z-10 text-center flex flex-col items-center dark:bg-[#0a0a0a] dark:border-[#222]">
                  <i class="pi pi-file-edit text-6xl mb-6 text-[#3b82f6] drop-"></i>
                  <span
                    class="bg-black text-white dark:bg-white dark:text-black px-4 py-2 border-4 border-black dark:border-[#222]">SELECT
                    A FILE TO EDIT</span>
                </div>
              </div>
            </div>
            ...
            <div v-show="isLivePreviewVisible"
              class="h-full bg-white flex flex-col border-l-4 border-black min-w-0 dark:bg-[#0a0a0a] dark:border-[#222]"
              ...
              <div
                class="flex-1 overflow-hidden bg-[#f4f4f0] dark:bg-[#050505] w-full h-full flex justify-center relative"
                :class="previewDevice !== 'desktop' ? 'p-4' : ''">
                <div
                  :class="['h-full bg-white dark:bg-[#0a0a0a] transition-all duration-300 flex flex-col', previewDevice === 'desktop' ? 'w-full border-none' : 'border-4 border-black overflow-hidden', previewDevice === 'tablet' ? 'w-[768px]' : previewDevice === 'mobile' ? 'w-[375px]' : '', 'dark:bg-[#0a0a0a] dark:border-[#222]']">
                  <webview ref="previewIframe" :src="previewUrl"
                    class="w-full h-full border-none bg-white flex-1 dark:bg-[#0a0a0a]" @did-navigate="handleNavigate"
                    @did-navigate-in-page="handleNavigate" @did-start-loading="handleStartLoading"
                    @did-stop-loading="handleStopLoading"></webview>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-show="isUserTerminalVisible"
            ...
            <div
              ...
              <div
                class="h-1 w-12 bg-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity dark:bg-[#0a0a0a]">
              </div>
            </div>
            <div
        ...
        <div v-show="isChatSidebarVisible"
          ...
          <div
            class="absolute left-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black dark:bg-[#0a0a0a] transition-colors z-50"
            @mousedown.prevent="startResizeRightSidebar">
          </div>

          <!-- Header: Chat Agent -->
          <div
            class="flex justify-between items-center border-b-4 border-black px-4 py-2 text-xs font-black uppercase tracking-widest transition-colors bg-[#3b82f6] dark:bg-[#111] text-white dark:border-[#222] shrink-0">
          ...
          <div
            class="px-3 py-2 bg-[#0d0d0d] border-b border-[#1e1e2e] flex items-center gap-2 shrink-0 text-xs text-white">
            <!-- Agent Selector -->
            ...
            <button @click="handleChatBrowseImage"
              ...
              title="Browse file, document, or image to attach">
              <i class="pi pi-paperclip text-xs"></i> Browse
            </button>
            ...
            <button @click="handleChatPasteImage"
              ...
              <i class="pi pi-image text-xs"></i> Paste
            </button>

            <!-- File / Image Badge -->
            <div v-if="chatUploadedImage"
              class="flex items-center gap-1.5 px-2 py-1 bg-[#0f172a] border border-[#1e40af] rounded-md text-xs font-mono text-[#93c5fd] max-w-[180px] truncate"
              :title="chatUploadedFileName">
              <i class="pi pi-paperclip text-xs text-[#93c5fd]"></i>
      ...
      <!-- Database Content -->
      <div v-show="activeMainTab === 'database'"
        class="flex-1 flex flex-col min-h-0 bg-white relative dark:bg-[#0a0a0a]">
        <DatabaseView />
      </div>
      ...
      <div v-if="isCtrlTabModalVisible"
        ...
        <div
          ...
          <div
            class="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-1 bg-[#f4f4f0] dark:bg-[#050505] border-4 border-black dark:border-[#222]">
      ...
      <div v-if="isRenameModalVisible" class="fixed inset-0 z-50 flex justify-center items-center pointer-events-none">
        ...
        <div
          ...
          <h3 class="font-black text-black uppercase mb-4 text-xl flex items-center gap-2 dark:text-gray-100"><i
      ...
      <div v-if="isNewFileModalVisible" class="fixed inset-0 z-50 flex justify-center items-center pointer-events-none">
        ...
        <div
          ...
          <h3
            class="font-black text-xl uppercase tracking-widest text-black border-b-4 border-black pb-2 mb-4 dark:text-gray-100 dark:border-[#222]">
            New File</h3>
...
const handleDiffEditorMount = (diffEditor: any) => {
...
const captureCurrentScrollState = () => {
  ...
  if (diffEditorRef.value) {
    try {
      const mod = diffEditorRef.value.getModifiedEditor?.()
      if (mod) {
        scrollTop = mod.getScrollTop?.() || 0
        position = mod.getPosition?.() || null
        const visibleRanges = mod.getVisibleRanges?.()
        if (visibleRanges && visibleRanges.length > 0) {
          lineNumber = visibleRanges[0].startLineNumber
        }
      }
    } catch (e) {
      console.warn("Failed to get scroll from diff editor:", e)
    }
  }

  if (!scrollTop && !lineNumber && monacoEditorRef.value) {
    try {
      scrollTop = monacoEditorRef.value.getScrollTop?.() || 0
      position = monacoEditorRef.value.getPosition?.() || null
      const visibleRanges = monacoEditorRef.value.getVisibleRanges?.()
      if (visibleRanges && visibleRanges.length > 0) {
        lineNumber = visibleRanges[0].startLineNumber
      }
    } catch (e) {
      console.warn("Failed to get scroll from monaco editor:", e)
    }
  }

  return { scrollTop, lineNumber, position }
}

const restoreMonacoScroll = (state: { scrollTop: number, lineNumber: number, position: any }) => {
  if (!state || (!state.scrollTop && !state.lineNumber && !state.position)) return
  ...
  const apply = () => {
    ...
    try {
      ...
      if (state.scrollTop > 0) {
        monacoEditorRef.value.setScrollTop(state.scrollTop)
      } else if (state.lineNumber > 0) {
        monacoEditorRef.value.revealLineInCenter(state.lineNumber)
      }
      if (state.position) {
        monacoEditorRef.value.setPosition(state.position)
      }
    } catch (e) {
      console.warn("Failed to restore monaco scroll:", e)
    }
  }
...
const openDiff = (path: string) => {
  let prevScroll = 0
  let prevLine = 0
  if (monacoEditorRef.value && selectedFile.value === path) {
    try {
      prevScroll = monacoEditorRef.value.getScrollTop?.() || 0
      const visibleRanges = monacoEditorRef.value.getVisibleRanges?.()
      if (visibleRanges && visibleRanges.length > 0) {
        prevLine = visibleRanges[0].startLineNumber
      }
    } catch (e) {
      console.warn("Failed to read scroll for diff mode:", e)
    }
  }
  ...
  if (prevScroll > 0 || prevLine > 0) {
    const applyDiffScroll = () => {
      if (diffEditorRef.value) {
        try {
          const mod = diffEditorRef.value.getModifiedEditor?.()
          const orig = diffEditorRef.value.getOriginalEditor?.()
          if (mod) {
            if (prevScroll > 0) mod.setScrollTop(prevScroll)
            else if (prevLine > 0) mod.revealLineInCenter(prevLine)
          }
          if (orig) {
            if (prevScroll > 0) orig.setScrollTop(prevScroll)
            else if (prevLine > 0) orig.revealLineInCenter(prevLine)
          }
        } catch (e) {
          console.warn("Failed to apply diff scroll:", e)
        }
      }
    }
    nextTick(() => {
      setTimeout(applyDiffScroll, 50)
      setTimeout(applyDiffScroll, 150)
    })
  }
}

const exitDiffMode = (capturedState?: { scrollTop: number, lineNumber: number, position: any }) => {
  const state = capturedState || captureCurrentScrollState()
  isDiffMode.value = false
  currentDiffFile.value = ''
  if (state.scrollTop > 0 || state.lineNumber > 0 || state.position) {
    restoreMonacoScroll(state)
  }
}
...
const approveChange = async () => {
  ...
  // Capture current scroll before diff mode exits or model updates
  const scrollState = captureCurrentScrollState()
  ...
  exitDiffMode(scrollState)

  // Otomatis trigger Ctrl+S (save to disk)
  await saveToDisk()
}
...
const rejectChange = async () => {
  ...
  try {
    ...
    broadcastToPlugins('editor.onSave', { code: change.original, path: absolutePath })
  } catch (err) {
  ...
  filesContent.value = {
    ...filesContent.value,
    [path]: change.original
  }

  if (unsavedChanges.value[path] !== undefined) {
    const updatedUnsaved = { ...unsavedChanges.value }
    delete updatedUnsaved[path]
  ...
  if (selectedFile.value === path) {
    if (monacoEditorRef.value) {
      ...
      if (model) {
        ...
        activeCode.value = change.original
      }
    } else {
      activeCode.value = change.original
    }
...
const sendSelectionToAgent = (targetEditor?: any) => {
  const ed = targetEditor || monacoEditorRef.value
  let targetTerminalId = CHAT_TERM_ID

  // Jika panel plugin sedang aktif, arahkan ke terminal plugin tersebut
  if (isPluginRightSidebarVisible.value && activeRightPluginId.value === 'btn_claude') {
    ...
    // Default arahkan ke Chat Agent native
    targetTerminalId = CHAT_TERM_ID
  ...
  if (ed) {
    ...
    if (selection && !selection.isEmpty()) {
      ...
      if (model) {
        ...
        const delay = (targetTerminalId === CHAT_TERM_ID && !chatAgentXterm) ? 800 : 300
        setTimeout(() => {
          ...
          if (targetTerminalId === CHAT_TERM_ID && chatAgentXterm) {
            chatAgentXterm.focus()
          }
        }, delay)
        return
      }
  ...
  if (targetTerminalId === CHAT_TERM_ID) {
    setTimeout(() => {
...
const getImageUploadDir = async () => {
  ...
  const targetDir = `${baseDir}${separator}.makarya${separator}attachments`
...
const refreshExplorer = async () => {
  ...
  if (!quickOpenQuery.value) {
    quickOpenResults.value = allFilesResult
  }
}
...
onMounted(() => {
  ...
  if (window.electronAPI && window.electronAPI.onFileTreeChange) {
    window.electronAPI.onFileTreeChange(() => {
      if (isProjectOpen.value && projectPath.value) {
        refreshExplorer()
      }
    })
  }
